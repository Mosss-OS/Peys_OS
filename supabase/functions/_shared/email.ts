/// Shared email notification logic for Peys Edge Functions.
  ...
  <div class="container">
    ...
    <div class="content">
      ...
      <div class="details">
        ...
        ${memo ? `<div class="details-row"><span>Note</span><span>"${memo}"</span></div>` : ""}
        <div class="details-row">
          <span>Expires</span>
...
export async function sendPaymentNotification(params: SendEmailParams): Promise<{ sent: boolean; error?: string }> {
  const { recipientEmail, senderEmail = "Someone", amount, token, memo, claimLink, appUrl = "https://peys.io" } = params;
  ...
  const htmlContent = generateEmailHTML({ recipientEmail, senderEmail, amount, token, memo, claimLink, appUrl });
  const textContent = generateEmailText({ recipientEmail, senderEmail, amount, token, memo, claimLink });
